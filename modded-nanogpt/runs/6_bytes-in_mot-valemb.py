"""
Altered from https://github.com/KellerJordan/modded-nanogpt/blob/master/train_gpt_medium.py
(version from Juli 12th, 2025).

Changes:

- Replaced the dataloader with the MoT dataloader
- Bytes at the input, MoT at the value embeddings
"""
...
@torch.compile(dynamic=False)
def reshape_bytes(byte_embs: Tensor):
    # equivalent to einops.rearrange(byte_embs, "B (S bpt) D -> B S (bpt D)", bpt=16)
    B, SB, D = byte_embs.shape
    bpt = 16
    S = SB // bpt
...
reshape_bytes = reshape_bytes  # make sure this function is compiled before the model -> avoids shape error
...
class GPT(nn.Module):
    def __init__(
        ...
        self.value_embeds_toks = nn.ModuleList([nn.Embedding(token_vocab_size, token_dim) for _ in range(3)])
        self.value_embeds_bytes = nn.ModuleList([nn.Embedding(token_vocab_size, byte_dim) for _ in range(3)])
        self.value_byte_mixin_weights = nn.ParameterList([nn.Parameter(init_linear(torch.empty(token_dim, token_dim + 16 * byte_dim)).bfloat16()) for _ in range(3)])
    ...
    def forward(self, token_inputs: Tensor, byte_inputs: Tensor, token_targets: Tensor, sliding_window_num_blocks: Tensor):
        ...
        x = x0 = norm(reshape_bytes(self.embed_bytes(byte_inputs).squeeze()[None]))
...
if master_process:
    ...
    os.makedirs("../logs/6_bytes-in_mot-valemb", exist_ok=True)
    logfile = f"../logs/6_bytes-in_mot-valemb/{run_id_full}.txt"
    print(logfile)
...
embed_params = [*model.embed_bytes.parameters(), *model.value_embeds_toks.parameters(), *model.value_embeds_bytes.parameters()]
scalar_params = [model.scalars]
head_params: list[nn.Parameter] = [model.lm_head_w]
mixin_params = [*model.value_byte_mixin_weights]
# sanity check
