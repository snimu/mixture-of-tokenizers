"""
Variation of 71041_mot-in_toks-valemb.py

Changes:

- norm the norm-scalars to sum to 1
...
if master_process:
    ...
    os.makedirs("../logs/71042_mot-in_toks-valemb", exist_ok=True)
    logfile = f"../logs/71042_mot-in_toks-valemb/{run_id_full}.txt"
    print(logfile)
...
for step in range(train_steps + 1):
    ...
    if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
        ...
        print0(f"#71042: step:{step}/{train_steps} val_loss:{val_loss:.6f} train_time:{training_time_ms:.0f}ms step_avg:{training_time_ms/max(step, 1):.2f}ms", console=True)
        token_lambda = model.scalars[-1]
        byte_lambda = model.scalars[-2]
        print0(f"#71042: {token_lambda=:.4f}, {byte_lambda=:.4f}", console=True)
        model.train()
