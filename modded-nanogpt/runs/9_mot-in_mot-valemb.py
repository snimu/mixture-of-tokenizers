...
class GPT(nn.Module):
    def __init__(
        ...
        self.byte_mixin_weight = nn.Parameter(init_linear(torch.empty(model_dim, token_dim + 16 * byte_dim)).bfloat16())
...
if master_process:
    ...
    os.makedirs("../logs/9_mot-in_mot-valemb", exist_ok=True)
    logfile = f"../logs/9_mot-in_mot-valemb/{run_id_full}.txt"
    print(logfile)
...
embed_params = [*model.embed_tokens.parameters(), *model.embed_bytes.parameters(), *model.value_embeds_toks.parameters(), *model.value_embeds_bytes.parameters()]
scalar_params = [model.scalars]
head_params: list[nn.Parameter] = [model.lm_head_w]
mixin_params = [model.byte_mixin_weight, *model.value_byte_mixin_weights]
# sanity check
