...
def make_token_to_bytes_embedding(vocab_size: int) -> nn.Embedding:
    ttb_emb = nn.Embedding(vocab_size, 8)
    with open("embeddings/ttb_8_left_pad.json", "r") as f:
        text = f.read()
    ttb = json.loads(text)
...
for step in range(warmup_steps):
    ...
    byte_inputs = torch.randint(0, args.byte_vocab_size, size=(args.train_seq_len * 8,), device="cuda").view(8, -1).contiguous()
