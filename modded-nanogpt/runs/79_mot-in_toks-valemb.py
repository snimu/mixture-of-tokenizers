"""
Variation of 79_mot-in_toks-valemb.py
...
def distributed_data_generator(filename_pattern: str, batch_size: int, rank : int, world_size : int, vocab_size: int):
    ...
    crnt_file = next(file_iter)
    tokens, pos = _load_data_shard(crnt_file), 0
    while True:
        ...
        yield token_inputs, byte_inputs.to(dtype=token_inputs.dtype), token_targets, crnt_file, num_total, num_pulled, num_blocked
...
if master_process:
    ...
    os.makedirs("../logs/77_mot-in_toks-valemb", exist_ok=True)
    logfile = f"../logs/77_mot-in_toks-valemb/{run_id_full}.txt"
    print(logfile)
...
# sanity check
params_collections = [hidden_matrix_params, embed_tokens_params, embed_bytes_params, scalar_params, head_params, mixin_params]
...
adam_param_groups = [
    ...
    dict(params=embed_bytes_params, lr=0.1),
...
for step in range(train_steps + 1):
    ...
    if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
        ...
        with torch.no_grad():
            for _ in range(val_steps):
                token_inputs, byte_inputs, targets, _, _, _, _ = next(val_loader)
    ...
    # --------------- TRAINING SECTION -----------------
    token_inputs, byte_inputs, targets, crnt_file, num_total, num_pulled, num_blocked = next(train_loader)
