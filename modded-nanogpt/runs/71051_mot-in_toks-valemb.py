"""
Variation of 71011_mot-in_toks-valemb.py

Changes:

- linear layer on bytes before summing tokens and bytes
...
def mixin_bytes(token_embs: Tensor, byte_embs: Tensor, byte_fc: Tensor):
    byte_embs = torch.cat([b for b in byte_embs], dim=-1)[None]
    byte_embs = F.linear(byte_embs, byte_fc)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.byte_fc = nn.Parameter(init_linear(torch.empty(model_dim, model_dim)).bfloat16())  # works on the concatenated bytes -> model_dim
    ...
    def forward(self, token_inputs: Tensor, byte_inputs: Tensor, token_targets: Tensor, sliding_window_num_blocks: Tensor):
        ...
        x = x0 = mixin_bytes(x_toks, x_bytes, self.byte_fc)
...
if master_process:
    ...
    os.makedirs("../logs/71051_mot-in_toks-valemb", exist_ok=True)
    logfile = f"../logs/71051_mot-in_toks-valemb/{run_id_full}.txt"
    print(logfile)
...
hidden_matrix_params.append(model.byte_fc)
...
for step in range(train_steps + 1):
    ...
    if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
        ...
        print0(f"# 71051: step:{step}/{train_steps} val_loss:{val_loss:.6f} train_time:{training_time_ms:.0f}ms step_avg:{training_time_ms/max(step, 1):.2f}ms", console=True)
        model.train()
