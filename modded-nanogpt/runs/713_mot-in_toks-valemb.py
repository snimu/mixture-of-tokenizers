"""
Variation of 711_mot-in_toks-valemb.py

Changes:

- Switched MLP and Attention layer order; because with the MoT-concat, the model-dim is already dependent on multiple tokens in the sequence (as opposed to normal token embeddings)
"""
...
def mixin_bytes(token_embs: Tensor, byte_embs: Tensor):
    ...
    return norm(x)  # use of norm here by @Grad62304977
...
if master_process:
    ...
    os.makedirs("../logs/713_mot-in_toks-valemb", exist_ok=True)
    logfile = f"../logs/713_mot-in_toks-valemb/{run_id_full}.txt"
    print(logfile)
