"""
Variation of 71_mot-in_toks-valemb.py
...
if master_process:
    ...
    os.makedirs("../logs/71034_mot-in_toks-valemb", exist_ok=True)
    logfile = f"../logs/71034_mot-in_toks-valemb/{run_id_full}.txt"
    print(logfile)
...
adam_param_groups = [
    ...
    dict(params=embed_tokens_params, lr=0.35),
    dict(params=embed_bytes_params, lr=0.45),
...
for step in range(train_steps + 1):
    ...
    if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
        ...
        print0(f"#71034: step:{step}/{train_steps} val_loss:{val_loss:.6f} train_time:{training_time_ms:.0f}ms step_avg:{training_time_ms/max(step, 1):.2f}ms", console=True)
        model.train()
