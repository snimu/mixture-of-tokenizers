"""
Variation of 71042_mot-in_toks-valemb.py

Changes:

- Initialize token_lambda=0.99, byte_lambda=0.01
...
class GPT(nn.Module):
    def __init__(
        ...
        self.scalars = nn.Parameter(torch.cat([
            ...
            *[torch.tensor([0.5, 0.5]) for _ in range(num_layers)], # SA lambdas
            torch.tensor([0.01, 0.99]), # embeddings norm lambdas
        ]))
    ...
    def forward(self, token_inputs: Tensor, byte_inputs: Tensor, token_targets: Tensor, sliding_window_num_blocks: Tensor):
        ...
        norm_scalrs_sum = self.scalars[-1] + self.scalars[-2]
        x_toks = norm(self.embed_tokens(token_inputs)[None]) * self.scalars[-1] / norm_scalrs_sum
        x_bytes = norm(self.embed_bytes(byte_inputs).squeeze()) * self.scalars[-2] / norm_scalrs_sum
...
if master_process:
    ...
    os.makedirs("../logs/71043_mot-in_toks-valemb", exist_ok=True)
    logfile = f"../logs/71043_mot-in_toks-valemb/{run_id_full}.txt"
    print(logfile)
...
embed_tokens_params = [*model.embed_tokens.parameters(), *model.value_embeds.parameters()]
embed_bytes_params = [*model.embed_bytes.parameters()]
scalar_params = [model.scalars]
head_params: list[nn.Parameter] = [model.lm_head_w]
# sanity check
params_collections = [hidden_matrix_params, embed_tokens_params, embed_bytes_params, scalar_params, head_params]
...
adam_param_groups = [
    ...
    dict(params=embed_tokens_params, lr=0.3),
    dict(params=embed_bytes_params, lr=0.3),
...
for step in range(train_steps + 1):
    ...
    if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
        ...
        print0(f"#71043: step:{step}/{train_steps} val_loss:{val_loss:.6f} train_time:{training_time_ms:.0f}ms step_avg:{training_time_ms/max(step, 1):.2f}ms", console=True)
        token_lambda = model.scalars[-1]
        byte_lambda = model.scalars[-2]
        print0(f"#71043: {token_lambda=:.4f}, {byte_lambda=:.4f}", console=True)
        model.train()
...
lambdas = model.scalars[1 * len(model.blocks): 3 * len(model.blocks)].view(-1, 2)
sa_lambdas = model.scalars[3 * len(model.blocks): 5 * len(model.blocks)].view(-1, 2)
token_lambda = model.scalars[-1]
byte_lambda = model.scalars[-2]
norm_scalrs_sum = token_lambda + byte_lambda
print0(
    s=(
        f"{skip_weights=}\n{lambdas=}\n{sa_lambdas=}\n"
        f"token_lambda={token_lambda / norm_scalrs_sum:.4f}\n"
        f"byte_lambda={byte_lambda / norm_scalrs_sum:.4f}\n"
    ),
    console=True,
)
