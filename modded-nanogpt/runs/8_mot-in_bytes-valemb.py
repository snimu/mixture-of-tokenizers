...
class GPT(nn.Module):
    ...
    def forward(self, token_inputs: Tensor, byte_inputs: Tensor, token_targets: Tensor, sliding_window_num_blocks: Tensor):
        ...
        # Byte embeddings as value embeddings
        ve = [reshape_bytes(value_embed(byte_inputs).squeeze()[None]) for value_embed in self.value_embeds_bytes]
...
if master_process:
    ...
    os.makedirs("../logs/8_mot-in_bytes-valemb", exist_ok=True)
    logfile = f"../logs/8_mot-in_bytes-valemb/{run_id_full}.txt"
    print(logfile)
...
embed_params = [*model.embed_tokens.parameters(), *model.embed_bytes.parameters(), *model.value_embeds_bytes.parameters()]
