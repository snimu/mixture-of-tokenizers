...
def make_token_to_bytes_embedding(vocab_size: int) -> nn.Embedding:
    ttb_emb = nn.Embedding(vocab_size, 32)
    with open("../embeddings/ttb_32_left_pad.json", "r") as f:
        text = f.read()
    ttb = json.loads(text)
...
if master_process:
    ...
    os.makedirs("../logs/71011_mot-in_toks-valemb", exist_ok=True)
    logfile = f"../logs/71011_mot-in_toks-valemb/{run_id_full}.txt"
    print(logfile)
...
for step in range(warmup_steps):
    ...
    byte_inputs = torch.randint(0, args.byte_vocab_size, size=(args.train_seq_len * 32,), device="cuda").view(32, -1).contiguous()
