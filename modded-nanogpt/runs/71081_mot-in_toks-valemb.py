...
- split x0 into x0t and x0b and do weighted sum of x, x0t, x0b at every layer
"""
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, ve: Tensor | None, x0t: Tensor, x0b: Tensor, block_mask: BlockMask, lambdas: Tensor, sa_lambdas: Tensor):
        x = lambdas[0] * x + lambdas[1] * x0t + lambdas[2] * x0b
...
class GPT(nn.Module):
    ...
    def forward(self, token_inputs: Tensor, byte_inputs: Tensor, token_targets: Tensor, sliding_window_num_blocks: Tensor):
        ...
        x0t = norm(self.embed_tokens(token_inputs)[None])
        x0b = norm(self.embed_bytes(byte_inputs).squeeze())
        x0b = torch.cat([b for b in x0b], dim=-1)[None]
        ...
        x = x0t * self.scalars[-1] + x0b * self.scalars[-2]
        for i in range(len(self.blocks)):
            if i in skip_map:
                x = x + skip_weights[skip_map[i]] * skip_connections[skip_map[i]]
            x = self.blocks[i](x, ve[i], x0t, x0b, block_masks[i], lambdas[i], sa_lambdas[i])
...
if master_process:
    ...
    os.makedirs("../logs/71081_mot-in_toks-valemb", exist_ok=True)
    logfile = f"../logs/71081_mot-in_toks-valemb/{run_id_full}.txt"
    print(logfile)
