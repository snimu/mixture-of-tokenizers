"""
Variation of 0_train_gpt_medium.py

- Renames the lambdas to be less confusing
- Prints out all the lambdas at each validation step
"""
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, ve: Tensor | None, block_mask: BlockMask, ve_lambdas: Tensor):
        ...
        v = norm(v)
        if ve is not None:
            v = ve_lambdas[0] * v + ve_lambdas[1] * ve.view_as(v) # @KoszarskyB & @Grad62304977
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, ve: Tensor | None, x0: Tensor, block_mask: BlockMask, x0_lambdas: Tensor, ve_lambdas: Tensor):
        x = x0_lambdas[0] * x + x0_lambdas[1] * x0
        if self.attn is not None:
            x = x + self.attn(x, ve, block_mask, ve_lambdas)
        x = x + self.mlp(norm(x))
        return x
...
class GPT(nn.Module):
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        unet_lambdas = self.scalars[:len(self.blocks)]
        x0_lambdas = self.scalars[1 * len(self.blocks): 3 * len(self.blocks)].view(-1, 2)
        ve_lambdas = self.scalars[3 * len(self.blocks): 5 * len(self.blocks)].view(-1, 2)
        for i in range(len(self.blocks)):
            if i in unet_map:
                x = x + unet_lambdas[unet_map[i]] * unet_residuals[unet_map[i]]
            x = self.blocks[i](x, ve[i], x0, block_masks[i], x0_lambdas[i], ve_lambdas[i])
            unet_residuals.append(x)
...
for step in range(train_steps + 1):
    ...
    if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
        ...
        unet_lambdas = model.scalars[:len(model.blocks)]
        x0_lambdas = model.scalars[1 * len(model.blocks): 3 * len(model.blocks)].view(-1, 2)
        ve_lambdas = model.scalars[3 * len(model.blocks): 5 * len(model.blocks)].view(-1, 2)
        ...
        print0(f"step:{step}/{train_steps} unet_lambdas:{unet_lambdas.tolist()}", console=True)
        print0(f"step:{step}/{train_steps} x0_lambdas_0:{x0_lambdas[:, 0].tolist()}", console=True)
        print0(f"step:{step}/{train_steps} x0_lambdas_1:{x0_lambdas[:, 1].tolist()}", console=True)
        print0(f"step:{step}/{train_steps} ve_lambdas_0:{ve_lambdas[:, 0].tolist()}", console=True)
        print0(f"step:{step}/{train_steps} ve_lambdas_1:{ve_lambdas[:, 1].tolist()}", console=True)
        model.train()
